...
console.log('[API] Base URL:', BASE_URL) // remove after fix
...
API.interceptors.response.use(
  ...
  (error) => {
    ...
    // Normalize FastAPI error format
...
export const executionAPI = {
  start:   (data: any) => API.post('/execution/start', data),
...
export const progressAPI = {
  checkin: (data: any) => API.post('/progress/checkin', data),
  ...
  acceptBurnout: (data: any) => API.post('/progress/burnout/accept', data),
  undoBurnout: (data: any) => API.post('/progress/burnout/undo', data)
}

export const goalsAPI = {
  list: () => API.get('/goals/'),
  get: (goalId: string) => API.get(`/goals/${goalId}`),
  export: (goalId: string, format: string) => API.get(`/goals/export/${goalId}?format=${format}`, { responseType: 'blob' }),
